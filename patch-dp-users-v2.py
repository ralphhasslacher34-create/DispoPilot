...
MARKER = "DISPOPILOT_DP_USERS_AUTH_V2"

AUTH_JS = r'''
/* DISPOPILOT_DP_USERS_AUTH_V2 */
let dpAuthInfo=null;
let dpAuthInstalled=false;

function dpAuthSiteUrl(){
  const configured =
    (window.DP_SHAREPOINT && window.DP_SHAREPOINT.webUrl) ||
    (typeof config!=='undefined' && config && config.webUrl) ||
    '';
  if(!configured)throw new Error('SharePoint-Webadresse fehlt.');
...
async function dpAuthBoot(){
  ...
  try{
    ...
    let currentEmail=dpAuthNorm(
      (window.DP_SHAREPOINT && window.DP_SHAREPOINT.userEmail) ||
      (typeof config!=='undefined' && config && config.userEmail) ||
      ''
    );
    let currentName=String(
      (window.DP_SHAREPOINT && window.DP_SHAREPOINT.userDisplayName) ||
      (typeof config!=='undefined' && config && config.userDisplayName) ||
      ''
    ).trim();
    let currentLogin='';

    if(!currentEmail){
      const current=await dpAuthJson(web+"/_api/web/currentuser?$select=Title,Email,LoginName");
      currentEmail=dpAuthNorm(current.Email);
      currentName=currentName||String(current.Title||'').trim();
      currentLogin=dpAuthNorm(current.LoginName);
    }

    if(!currentEmail && !currentLogin){
      throw new Error('Angemeldetes Microsoft-365-Konto konnte nicht ermittelt werden.');
    }

    const users=await dpAuthJson(
      web+"/_api/web/lists/getbytitle('DP_Users')/items"+
      "?$select=Id,Title,Rolle,Aktiv,Fahrername,"+
      "Benutzer/Id,Benutzer/Title,Benutzer/EMail,Benutzer/Name"+
      "&$expand=Benutzer&$top=5000"
    );
    ...
    const row=(users.value||[]).find(x=>{
      ...
      return !!(
        (mail && currentEmail && mail===currentEmail) ||
    ...
    dpAuthInfo={
      role,
      driverName,
      displayName:String(currentName||row.Benutzer?.Title||row.Title||currentEmail||'Benutzer'),
      email:String(currentEmail||''),
      loginName:String(currentLogin||''),
      userId:Number(row.Benutzer?.Id||0)
    };
...
document.body.classList.add('dp-auth-pending');
void dpAuthBoot();
/* DISPOPILOT_DP_USERS_AUTH_V2_END */
'''
